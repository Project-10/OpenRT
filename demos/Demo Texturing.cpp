...
int main()
{
	...
	// Marble shader
	//CGradient gradientMarble(RGB(255, 255, 255), RGB(119, 135, 153));
	CGradient gradientMarble(Vec3f::all(1), Vec3f::all(0));
	//gradientMarble.addColor(0.75f, Vec3f::all(0));
	auto pTextureMarble = std::make_shared<CTextureMarble>(gradientMarble, 0.5f, 7,   6, 1.0f, 16.4f, 2.5f, 0.25f);
	auto pShaderMarble = std::make_shared<CShaderPhong>(*pScene, pTextureMarble, 0.1f, 0.9f, 0.5f, 20.0f);

	// Stripes shader
	auto pTextureStripes = std::make_shared<CTextureStripes>(5);
	auto pShaderStripes = std::make_shared<CShaderPhong>(*pScene, pTextureStripes, 0.1f, 0.9f, 0.5f, 80.0f);

	// Rings shader
	auto pTextureRings = std::make_shared<CTextureRings>(5);
	auto pShaderRings = std::make_shared<CShaderPhong>(*pScene, pTextureRings, 0.1f, 0.9f, 0.5f, 80.0f);
	...
	pScene->add(pCamera);
	 ...
	 auto sphere2 = CSolidSphere(pShaderMarble, Vec3f(0, 1, 0), 1.5f, 64);
	 //auto sphere2 = CSolidTorus(pShaderWood, Vec3f(0, 1, 0), 1.5f, 1.0f, 24);
	 auto sphere3 = CSolid(std::make_shared<CPrimSphere>(pShaderMarble, Vec3f(4, 1, 0), 1.5f));
	...
	//imshow("Stripes Texture", imgStripes);
	 ...
	 pScene = buildSceneTemplates(bgColor, resolution);
