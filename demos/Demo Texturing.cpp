...
int main() {
	...
	auto pShaderFloor	= std::make_shared<CShaderPhong>(scene, RGB(255, 255, 255), 0.1f, 0.9f, 0.0f, 40.0f);
 	auto pShaderFloor1	= std::make_shared<CShaderPhong>(scene1, RGB(255, 255, 255), 0.1f, 0.9f, 0.0f, 40.0f);
   	...
   	auto pShaderFloor3	= std::make_shared<CShaderPhong>(scene3, RGB(255, 255, 255), 0.1f, 0.9f, 0.0f, 40.0f);
	...
	auto pShaderRings	= std::make_shared<CShaderPhong>(scene3, pTextureRings, 0.1f, 0.9f, 0.0f, 40.0f);
	...
	scene.add(CSolidQuad(pShaderFloor, Vec3f::all(0), Vec3f(0, 1, 0), Vec3f(0, 0, 1), 500));
     ...
     scene3.add(CSolidQuad(pShaderFloor3, Vec3f::all(0), Vec3f(0, 1, 0), Vec3f(0, 0, 1), 500));
	...
	CSolid teapot(pShaderStripes, dataPath + "teapot.obj");
     ...
     CSolid teapot3(pShaderRings , dataPath + "teapot.obj");
     scene1.add(solidSphere2);
     scene2.add(solidBox);
    CTransform t;
	teapot.transform(t.scale(5.0f).get());
 	teapot3.transform(t.scale(5.5f).translate(0,-2,0).get());
     scene3.add(teapot3);
    scene.add(teapot);
   ...
   std::cout << x << std::endl;
    ...
    Mat img3 = scene3.render(std::make_shared<CSamplerStratified>(2, true, true));

    Timer::stop();
    imshow("Stripes Texture", img);
    imshow("Wood Texture", img1);
    imshow("Marble Texture", img2);
    imshow("Rings Texture", img3);
    waitKey();
    return 0;
}
