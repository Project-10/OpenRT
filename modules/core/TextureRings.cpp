...
namespace rt{
          ...
          Vec3f CTextureRings::woodMap(float value) const{
	...
	Vec3f CTextureRings::getTexel(const Vec3f& uvw) const
	{
		...
		float w = uvw.val[2];
          ...
          double n = 0.6 * pn.noise(0.55*u, 0.45*v ,0.35*w);
          //n = n - floor(n);
    
		//Difference between point and center of the shape
		float value = m_period * sqrt(w * w + u * u) + n;
  
          //Perlin Noise
          Vec3f color = woodMap(remainder(value, 1) );
		return color;
	}
...
//
//		//Difference between point and center of the shape
//		int value = m_period * w * w + u * u;
//
//          Vec3f color = woodMap(value % 1);
//
//		return color;
//
//      }
