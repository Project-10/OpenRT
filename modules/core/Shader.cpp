...
namespace rt {
    ...
    Vec3f CShader::getDiffuseColor(const Ray &ray) const {
        ...
        if (m_pDiffuseColorMap) {
            return m_pDiffuseColorMap->isProcedural() ? m_pDiffuseColorMap->get3DTexel(ray.hitPoint())
    ...
    float CShader::getSpecularLevel(const Ray &ray) const {
        return m_pSpecularLevelMap ? m_specularLevel * m_pSpecularLevelMap->getTexel(ray.hit->getTextureCoords(ray))[0]
                                   : m_specularLevel;
    }
}
