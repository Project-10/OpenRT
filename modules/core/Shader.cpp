...
namespace rt {
	...
	Vec3f CShader::getAmbientColor(const Ray& ray) const
	{
        ...
        if(m_pDiffuseColorMap){
            return m_pDiffuseColorMap->isProcedural() ? m_pDiffuseColorMap->getTexel(ray.hit->wcs2ocs(ray.hitPoint()))
	...
	float CShader::getSpecularLevel(const Ray& ray) const
	{
        ...
        if(m_pDiffuseColorMap){
            return m_pDiffuseColorMap->isProcedural() ? m_pDiffuseColorMap->getTexel(ray.hit->wcs2ocs(ray.hitPoint()))[0]
