...
namespace rt {
	...
	) : CSolid(0.25f * (a + b + c + d)), m_solid(solid)
	{
		add(std::make_shared<CPrimTriangle>(pShader,solid ,a, b, c, ta, tb, tc, na, nb, nc));
		add(std::make_shared<CPrimTriangle>(pShader,solid ,a, c, d, ta, tc, td, na, nc, nd));
	}
}
