...
namespace rt {
	...
	public:
		 ...
		 */
		DllExport CLightSky(Vec3f intensity, float radius = std::numeric_limits<float>::infinity(), std::shared_ptr<CSampler3f> pSampler = std::make_shared<CSampler3f>(std::make_shared<CSamplerStratified>(4, true, true), 10), bool castShadow = true)// std::make_shared<CSampler3fTangent>(4,2.0f,-1.0f))
			: ILight(castShadow)
		...
		DllExport virtual std::optional<Vec3f> illuminate(Ray& ray) override
		{
			...
			ray.dir = m_pSampler->getNextSample(normal);
