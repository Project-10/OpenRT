...
namespace rt {
    ...
    CTexture::CTexture(const Mat &img) : Mat(img) {
        if (!empty()) {
            RT_ASSERT_MSG(img.channels() == 3,
                          "Can't create texture from %d-channels images. A 3-channels image is needed.",
                          img.channels());
    ...
    // Reason: the isProcedural call always returns false, and if its false, then it means we shouldn't try to use
    // the 3D mapping.
    Vec3f CTexture::get3DTexel(const Vec3f &ray) {
